...
This cheap(est) USB sound card works pretty well on macOS _without_ requiring
...
-qemu-system-x86_64 -enable-kvm -m 16384 -cpu Penryn,kvm=off,vendor=GenuineIntel \
+qemu-system-x86_64 -enable-kvm -m 16384 -cpu Penryn,kvm=off,vendor=GenuineIntel,+aes \
          -machine pc-q35-2.4 \
          -smp 4,cores=2 \
...
_Note: If using NVMe, passing the controller may be a better option then passing it as a block device_
